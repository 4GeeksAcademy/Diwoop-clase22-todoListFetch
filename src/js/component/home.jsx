...
const Home = () => {
    ...
    const getUserTasks = () => {
        fetch("https://playground.4geeks.com/todo/users/diana_estallo")
        .then (resp => {
            console.log(resp);
            if (resp.ok === false) {
                creatUser()
    ...
    const createTask = (task) => {
        fetch("https://playground.4geeks.com/todo/todos/diana_estallo", {
            method: "POST",
            headers: {
                "Content-Type":"application/json"
            },
    ...
    <div className="bg-light d-flex flex-column justify-content-start align-items-center min-vh-100">
        ...
        <div className="card bg-light rounded-0 shadow w-50">
            <div className="card-body text-justify">
				...
				<button className="rounded-1 border-0 bg-danger-subtle" onClick={deleteAll}>Delete all tasks</button>
				</div>
            </div>
        </div>
    </div>
